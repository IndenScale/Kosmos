...
class DocxParser(DocumentParser):
    ...
    def _parse_via_pdf_conversion(self, file_path: str) -> List[ParsedFragment]:
        """通过转换为PDF来解析DOCX文件"""
        # 创建临时目录 - 使用绝对路径确保在任何工作目录下都能正确创建
        temp_id = str(uuid.uuid4())
        project_root = Path(__file__).parent.parent.parent  # 获取项目根目录
        temp_dir = project_root / "data" / "temp" / "docx_conversion" / temp_id
    ...
    def _convert_to_pdf(self, docx_path: str, output_dir: Path) -> str:
        ...
        try:
            self.logger.info(f"使用 LibreOffice 转换 DOCX 到 PDF...")
            self._convert_with_libreoffice(local_path, str(pdf_path))

            # 检查期望的PDF文件是否存在
            if pdf_path.exists():
                self.logger.info(f"DOCX转PDF成功 (LibreOffice): {local_path} -> {pdf_path}")
                return str(pdf_path)
            ...
            if output_dir.exists():
                ...
                if pdf_files:
                    ...
                    self.logger.info(f"找到生成的PDF文件: {actual_pdf}")
                    # 如果文件名不同，重命名为期望的文件名
                    if actual_pdf != pdf_path:
                        actual_pdf.rename(pdf_path)
                        self.logger.info(f"PDF文件已重命名为: {pdf_path}")
                    return str(pdf_path)

            self.logger.warning(f"LibreOffice 转换完成但PDF文件不存在")
            raise Exception("LibreOffice转换完成但PDF文件不存在")

        except Exception as e:
            self.logger.error(f"LibreOffice 转换失败: {str(e)}")
    ...
    def _convert_with_libreoffice(self, docx_path: str, pdf_path: str):
        ...
        # 调试信息：检查输入文件和输出目录
        # print(f"LibreOffice转换调试信息:")
        # print(f"  输入文件: {docx_path}")
        # print(f"  输入文件存在: {Path(docx_path).exists()}")
        # print(f"  输入文件大小: {Path(docx_path).stat().st_size if Path(docx_path).exists() else 'N/A'} bytes")
        # print(f"  输出目录: {output_dir}")
        # print(f"  输出目录存在: {output_dir.exists()}")
        # print(f"  期望PDF路径: {pdf_path}")
